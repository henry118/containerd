...
func WithCapabilityCgroupPath(cgroupPath string) AttachmentOpt {
	return func(c *AttachmentArgs) error {
...
func (a *AttachmentArgs) config(ns string) *libcni.RuntimeConf {
	c := &libcni.RuntimeConf{
		ContainerID: strings.Join([]string{ns, a.ContainerID}, "."),
		NetNS:       a.NSPath,
		IfName:      a.IFName,
	}
