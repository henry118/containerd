...
func TestNetworkAttachment(t *testing.T) {
	...
	assert.Equal(t, c.rt.ContainerID, fmt.Sprintf("%s.%s", TestNamespace, TestContainer))
	assert.Equal(t, c.rt.IfName, TestInterface)
	assert.Equal(t, c.rt.NetNS, TestNSPath)
